...
export const MacbookScroll = ({
  ...
  // Phase 1 (0 → 0.4): Lid opens, screen scales up to fill viewport
  // We reduced 2.5 to 1.8 here because the wrapper is now scaled up (up to 1.4x), 
  // so 1.4 * 1.8 = ~2.5 total scale to avoid over-exaggeration.
  ...
  // Translate down initially to stay in viewport as it grows,
  // then stop translating so it naturally scrolls up out of the viewport.
  // Reduced to 550 since it is inside a scaled wrapper.
  ...
  // Fade out slightly as it scrolls off screen
  ...
  return (
    <div
      ref={ref}
      className="flex min-h-[220vh] flex-col items-center justify-start py-0 md:py-80 overflow-hidden"
    >
      <motion.h2
        style={{
        ...
        className="mb-20 text-center text-3xl font-bold text-white px-4"
      >
        {title || (
          <span>
      ...
      {/* MacBook Hardware Wrapper (Scaled up) */}
      <div className="shrink-0 scale-[0.4] sm:scale-75 md:scale-[1.2] lg:scale-[1.4] transform [perspective:800px] origin-top">
        {/* Lid */}
        <Lid
          src={src}
          scaleX={scaleX}
...
export const Lid = ({
  ...
  return (
    <div className="relative [perspective:800px]">
      {/* Static tilted lid back (visible when "closed") */}
      <div
        style={{
      ...
      {/* Animated screen that opens, scales up, and slides away */}
      <motion.div
        style={{
          scaleX: scaleX,
          scaleY: scaleY,
      ...
      >
        ...
        {children ? (
          <div className="absolute inset-0 rounded-lg overflow-hidden">
            {/* 
              Render children at 1024×768 native resolution, 
              scaled down to fit the 512×384 MacBook screen.
              This prevents pixelation when the Aceternity animation 
              scales the screen up — it's approaching native res, not stretching.
            */}
            <div
          ...
          <img
            src={src}
            alt="StackAudit screen"
            className="absolute inset-0 h-full w-full rounded-lg object-cover object-left-top"
          />
        ) : (
          <div className="absolute inset-0 rounded-lg bg-[#0a0a0c] flex items-center justify-center">
            <span className="text-white/40 font-mono text-sm">StackAudit</span>
          </div>
        )}
      </motion.div>
    </div>
